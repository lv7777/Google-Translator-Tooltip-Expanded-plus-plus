// ==UserScript==
// @name			Google Translator Tooltip Expanded plus plus
// @namespace		lv7777
// @description		Google Translator Tooltip Expandedを拡張したものです。
// @version			0.a
// @include			*
// @grant GM_getValue
// @grant GM_setValue
...
//DeleteTangotyou();
//GM_deleteValue("testdata");
//registerTangotyou("datadatadata");
GM_registerMenuCommand("単語帳を開く", My_open_tangotyou);
...
function lookup(evt)
{
    ...
    if(GM_getValue("My_tangotyou_auto_add")==true){//MyCode
        registerTangotyou(txtSel.toString());
    }
    ...
    //単語帳追加ボタン
    var TangoButtonDataURI ="data:image/gif;base64,R0lGODlhEAAQAKIEANTQyAAAAP//AICAgP///wAAAAAAAAAAACwAAAAAEAAQAAADPAi63P6QhdiCmBRYcenm3fNxwSCCJFFW6EUMQbyMbjyopvIFOOGvs0vMRwTqhsRi7hhrOpeAgXRKhWYWCQA7"; 
    //hiddenは論理属性（attr名しか記述しないやつ）
    var registTangoButton=createElement("img",{"id":"registTango","class":"my___class","title":"単語帳に追加します","accesskey":"T"/*Firefox 2+  Alt+Shift+アクセスキー*/,"tabindex":"-50"/*tabキーによるアクセス不可,"hidden":"false"*/,"translate":"false","contentable":"true","style":"","src":TangoButtonDataURI},null,null);
    //GM_log(registTangoButton);
    //  var registTangoButton=createElement("img",{id:"registTango",title:"単語帳に追加します",accesskey:"t"/*Firefox 2+  Alt+Shift+アクセスキー*/,tabindex:"-50"/*tabキーによるアクセス不可*/,hidden:"true",translate:"false",style:""})
//    var grovalAttr=["id","class","dir","lang","title","spellcheck","accesskey","contentable","tabindex","hidden","trancelate","style"];
    registTangoButton.addEventListener("click", function(){registerTangotyou(txtSel); reviewanime();}, false);
    divDic.appendChild(registTangoButton);
...
function extractResult(gTradStringArray)
{
	...
	*/
    ...
    Speak(getId('texttospeachbuttonfrom'), arr[2], fromText); // arr[2] contains the detected input language
    //Speak(getId('texttospeachbuttonto'), GM_getValue('to', 'auto') == 'auto' ? 'en' : GM_getValue('to', 'auto'), toText); // I cannot find a way to get the detected destination language, so if the requested destination is 'auto', I use the english Text to Speach language
...
function Speak(element,lang,text){
    
    if(GM_getValue("My_auto_speak")==false){
        return 0;
    }
    
    var urlspase='http://translate.google.com/translate_tts?tl='+lang+'&q='+text;
    var url='http://translate.google.com/translate_tts?tl='+lang+'&q='+text.replace(/[«»'"]/g,' ');
    GM_log(url);
    var speachLink = createElement('audio',{id:"MyAudioplus",src:urlspase,autoplay:"autoplay"});
    GM_log(speachLink);
...
function addTextToSpeachLink(element, lang, text) {
   ...
   // speachLink.innerHTML = '<img src="http://www.trace-de-son.com/images/sound-icon.png" height="16" width="16"/>';
	speachLink.innerHTML = '<img src="http://emoji.ameba.jp/img/user/ga/gakupo11181119/3657205.gif" height="16" width="16"/>';
    element.appendChild(speachLink);
}
...
function openCloseOptions(evt)
{
    ...
    if(!divOptions) //Show options
    {
        ...
        //MY_auto_tangotyou 
        divOptionsFields.appendChild(createElement('br'));
        divOptionsFields.appendChild(createElement('input', {id:'autotango', type:'checkbox'}));
        divOptionsFields.appendChild(createElement('span', null, null,'単語帳自動登録'));
        getId('autotango').checked = GM_getValue('My_tangotyou_auto_add');
        
        //MY_auto_speak
        divOptionsFields.appendChild(createElement('br'));
        divOptionsFields.appendChild(createElement('input', {id:'autospeak', type:'checkbox'}));
        divOptionsFields.appendChild(createElement('span', null, null,'音声自動再生'));
        getId('autospeak').checked = GM_getValue('My_auto_speak');
...
function saveOptions(evt){
    ...
    var alt = getId('checkAlt').checked;
  	var My_tangotyou_auto_add=getId('autotango').checked;
    var My_auto_speak=getId('autospeak').checked;
    ...
    GM_setValue('alt', alt);
    GM_setValue('My_tangotyou_auto_add' , My_tangotyou_auto_add);
    GM_setValue('My_auto_speak', My_auto_speak);
    quickLookup();
...
function resetOptions(evt){
    ...
    GM_deleteValue('ctrl');
    GM_deleteValue('alt');
    GM_deleteValue('My_tangotyou_auto_add');
    GM_deleteValue('My_auto_speak');
...
//設定で選んだサイトからの翻訳にする
function MyselectTranslateSite(){
    
}
function getSitefromSetting(){
...
function My_trrigar_favolite(data){
    var arr=GM_getValue(data).split(" ");//空白で分割
    if(arr[3]=="true"){
        arr[3]="false";
    }else{
        arr[3]="true";
...
//ユーザーコマンドから起動
//GM_registerMenuCommand("単語帳を開く", My_open_tangotyou);
function My_open_tangotyou(){
    //方法その１
    //サーバ立てて専用のページ作る
    //方法その2
    //blankページとgreasemonkeyにhtml埋め込み
    //方法その3
    //localにページをおいてそれを持ってくる
    var w;
   w=GM_openInTab("https://www.google.com/");
   // GM_openInTab("file:///C:/Users/tes/AppData/Roaming/Mozilla/Firefox/Profiles/81e7hk04.dev-edition-default/gm_scripts/Google_Translator_Tooltip_Expanded/index.html")
   GM_log(w);
}
//単語帳の登録。autoでも手動でもこいつを呼ぶ
function registerTangotyou(text){//ok
    ...
    //登録内容
    //単語、回数、日時、サイト、、favorite(beta)
    if( (text === "") || ( text.match(/\s/) != null ) ){//空文字列 or 間に空白があるか//わかりやすい条件は先に書く
       return;
       }
      
    var cnt;//なぜかエラッタ
    if(cnt=GM_getValue(text)){//.split(" ");//なぜか誤反応した.search(/^*?(?=\s)/);//空白の前まで
        cnt=cnt[0];
        alert(cnt);
    }else{
        cnt=0;
     ...
     var date=new Date();
     var My_dateformat=date.getFullYear()+"/"+(date.getMonth() + 1)+"/"+date.getDate();
     var domain=document.domain;//空白怖いな
     var fav="false";
     GM_setValue("MY"+text, (cnt++) +" "+My_dateformat+" "+domain+" "+fav);
   //GM_log( (cnt++) +" "+My_dateformat+" "+domain+" "+fav);
   //GM_log(text);
}


//単語帳の取得。新しく開いたページ用
function getTangotyou(){
    var returnarr=[];//内部にobj
    var listarr=GM_listValues();
   
    listarr.forEach(function(data,i){
        if(data.match(/^MY/) == null){
            return;
        }
    //   GM_log(data);
       var arr=GM_getValue(data).split(" ");//空白で分割
...
//ページに<meta name=referrer>をset
//google音声APIにreferrerがgoogleじゃないとnotfoundが返されるのでそれを回避
//また、noreferrerはaタグとかにしか使えないので
function MyReferrerSet(){
    var head= getTag('head')[0];//配列なしだとHTMLコレクションだった
    GM_log(head);
    var metaE=document.createElement("meta");
     GM_log(metaE);
    metaE.setAttribute("name","referrer");
    metaE.setAttribute("content","never");
